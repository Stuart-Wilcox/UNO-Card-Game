...
class OrderedList {
public:
	OrderedList(unsigned int capacity = DEFAULT_LIST); // constructor for an empty ordered list that can hold//  up to capacity Ts; default size is defined in List.h
	...
	T remove(unsigned int pos); // removes the element at location pos from the list, and returns it;// PRECONDITION: pos must be a valid position                                             
	T retrieve(unsigned int pos) const; // returns value at location pos without modifying it; // PRECONDITION: pos must be a valid list position
