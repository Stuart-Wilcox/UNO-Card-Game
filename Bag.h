...
class Bag {
public:
	Bag(unsigned int max_capacity = DEFAULT_BAG); // construct an empty bag of specified "max_capacity"// if no value is provided, DEFAULT_BAG is used
	~Bag();  // destructor (recycles memory allocated by constructor)
	void add(T value); // adds a new T into the bag,  // PRECONDITION: the bag must not be "full"
	T getOne(); // extracts an T from the bag, // PRECONDITION: the bag must not be "empty"
	bool isEmpty() const;
	bool isFull() const;
